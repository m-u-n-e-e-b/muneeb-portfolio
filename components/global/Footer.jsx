            ...
            <div key={index} className="text-left mb-5 sm:mb-0">
              ...
              <div>
                {item.links.map((item, index) => {
                   const isEmail = item.name === "info-muneeb@proton.me";
                  return (
                    <div key={index} className="my-4">
                      {item.leavesWebsite ? (
                        <a
                          {...(!isEmail && { href: item.link, target: "_blank" })}
                          target="_blank"
                          className={`items-center flex ${isEmail ? 'cursor-text' : 'cursor-pointer'}`}
                        ...
                        !isEmail && <Link href={item.link}>{item.name}</Link> 
                      )}
                    </div>
                  );
                })}
